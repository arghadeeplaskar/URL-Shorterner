...
export default function RedirectPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  useEffect(() => {
    ...
    if (linkItem && linkItem.url) {
      ...
      setError("This link does not exist or was purged from your local cache.")
    }
  }, [code])
    ...
    <main className="min-h-screen relative flex items-center justify-center p-4 bg-background overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md glass-panel p-8 text-center rounded-2xl border border-white/10 shadow-2xl">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-foreground">
          D4XA1 <span className="text-gradient-purple">Dispatcher</span>
        </h1>

        {!mounted ? (
          <div className="mt-6 flex flex-col items-center gap-3">
            <div className="w-6 h-6 border-2 border-primary border-t-transparent rounded-full animate-spin" />
            <p className="text-sm text-muted-foreground font-mono">Resolving route /{code}…</p>
          </div>
        ) : target ? (
          <div className="mt-6 space-y-4">
            ...
            <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
              ...
              <span>Redirecting immediately…</span>
            </div>
          ...
          <div className="mt-6 space-y-4">
            ...
            <Link
              ...
              className="btn-glow inline-flex items-center justify-center gap-2 w-full py-2.5 text-sm font-medium rounded-xl"
            >
              <span>Return to Link Studio</span>
              <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        )}

        <div className="mt-6 pt-4 border-t border-border/40 text-[11px] text-muted-foreground flex items-center justify-center gap-1">
          <span>Protected by D4XA1 Core</span>
          <span>•</span>
